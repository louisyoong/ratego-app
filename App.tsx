...
const App: React.FC = () => {
  ...
  const generateHistory = useCallback(() => {
    ...
    for (let i = days; i >= 0; i--) {
      ...
      const dateLabel = timeRange === '1Y' 
        ? date.toLocaleDateString(language === 'en' ? 'en-US' : language, { month: 'short', year: '2-digit' })
  ...
  const handleAiAnalysis = async () => {
    setIsAiLoading(true);
    const trendString = chartData.slice(-10).map(d => d.rate.toFixed(4)).join(', ');
    const insight = await analyzeMarketTrends(base, target, rates[target], `Last points: [${trendString}]. Current range: ${timeRange}`, language);
  ...
  const renderContent = () => {
    switch(activeTab) {
      ...
      case 'terms':
        return <TermsView language={language} onBack={() => setActiveTab('home')} />;
      case 'privacy':
        return <PrivacyView language={language} onBack={() => setActiveTab('home')} />;
      case 'home':
      default:
        return (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            <div className="lg:col-span-7 space-y-8 animate-in fade-in slide-in-from-left-4 duration-500">
              <section className="space-y-4">
                <div className="flex items-center justify-between">
                  <h2 className="text-3xl font-bold text-white tracking-tight">{t.converter}</h2>
            ...
            <div className="lg:col-span-5 space-y-8 animate-in fade-in slide-in-from-right-4 duration-500">
              ...
              <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6">
                <h3 className="text-lg font-bold text-white mb-6">{t.popularConversions}</h3>
                <div className="space-y-4">
                  {['EUR', 'GBP', 'JPY', 'CNY', 'AUD'].filter(c => c !== base).slice(0, 5).map(code => (
                    <div key={code} className="flex items-center justify-between p-3 bg-slate-800/30 rounded-xl hover:bg-slate-800/50 transition-all cursor-pointer group">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="font-semibold text-slate-300">{base} / {code}</span>
                      </div>
                      <div className="text-right">
                        <p className="font-mono text-white">{(rates[code] || 0).toFixed(4)}</p>
              ...
              <div className="bg-gradient-to-br from-blue-600/10 to-purple-600/10 border border-blue-500/20 rounded-2xl p-6">
                ...
                <p className="text-sm text-slate-400 leading-relaxed mb-4">
                  Our AI-driven analytics provide a broad overview of currency trends. For professional investment decisions, please consult with a certified financial advisor.
                </p>
                <button 
    ...
    <div className="min-h-screen bg-[#0a0c10] text-slate-200 selection:bg-blue-500/30">
      ...
      <footer className="max-w-7xl mx-auto px-4 py-8 border-t border-slate-800 mt-12">
        <div className="flex flex-col md:flex-row justify-between items-center gap-4 text-xs text-slate-500 font-medium uppercase tracking-widest">
          <p>© 2024 RateGo Platform</p>
          <div className="flex gap-6">
            <button onClick={() => setActiveTab('privacy')} className="hover:text-white transition-colors">{t.privacy}</button>
            <button onClick={() => setActiveTab('terms')} className="hover:text-white transition-colors">{t.terms}</button>
            <a href="#" className="hover:text-white transition-colors">{t.apiStatus}</a>
          </div>
        </div>
      </footer>
    </div>
